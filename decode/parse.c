...
struct header *parseheaders(char *buf, int nbuf, int *pos, struct Time_Step_ *time, int *type, char **argv)
{
  struct header *h;
  int nc, nl, i, j, c, k = 0;
  char copie[10], *freq = 0, *date = 0, m_date[7], **id_sortie = calloc(17, sizeof(char *));

	for(i = 0; i < 17; i++)
		id_sortie[i] = calloc(1, sizeof(char));

	strcpy(id_sortie[0], "Ux");
	strcpy(id_sortie[1], "Uy");
	strcpy(id_sortie[2], "Uz");
	strcpy(id_sortie[3], "Tsonic");
	strcpy(id_sortie[4], "VWC");
	strcpy(id_sortie[5], "EC");
	strcpy(id_sortie[6], "Tsoil");
	strcpy(id_sortie[7], "Tair");
	strcpy(id_sortie[8], "Hair");
	strcpy(id_sortie[9], "Tdew");
	strcpy(id_sortie[10], "Patm");
	strcpy(id_sortie[11], "SWUpper");
	strcpy(id_sortie[12], "SWLower");
	strcpy(id_sortie[13], "LWUpper");
	strcpy(id_sortie[14], "LWLower");
	strcpy(id_sortie[15], "TC");
	strcpy(id_sortie[16], "TK");
      ...
      for(i = i+2, c = 0; c < nc; i++)
	{
      ...
      *pos = i ;
	  ...
	  for(i = 0; i < nc; i++)	
		for(j = 0; j < 17; j++)
		{
				if(Strncmp(h[i].name, id_sortie[j], strlen(id_sortie[j]))== 0)
				{
					h[i].id = j+1;
					break;
				}
...
int Strncmp(char *buf, char *tab, int n)
{
	int taille = strlen(buf);
	for(int i = 0; taille>0; taille--)
	{
		if(!strncmp(buf+i,tab,n))
			return 0;
		else
			buf++;
	}
...
char *increment_date(struct Time_Step_ *time, int freq)
{
  ...
  if(!strncmp(time->units, "MIN", 3))
	...
	i = 0;
  	sprintf(date, "%d-", time->annee);
	i+=5;
	rewrite_date(time->mois, date+i);
	i+=2;
	sprintf(date+i, "-");
	i++;
	rewrite_date(time->jour, date+i);
	i+=2;
	sprintf(date+i, " ");
	i++;
	rewrite_date(time->heure, date+i);
	i+=2;
	sprintf(date+i, ":");
	i++;
	rewrite_date(time->minute, date+i);
	i+=2;
	sprintf(date+i, ":");
	i++;
	rewrite_date(time->seconde, date+i);
	i+=2;
	sprintf(date+i, ".");
	i++;
	rewrite_date(time->tiece, date+i);

    return date;
}

void rewrite_date(int val, char *date)
{
	char hipe[4];
...
void parsevalues(struct header *h, char *tmp, int *pos, int size, int input, int output, struct Time_Step_ *time, char *date)
{
  ...
  char seq[25], buf[25], seq_id_station[25], *id = calloc(3, sizeof(char));
  infd seq_decode;
  int p = 0, datapos = 0;
  ...
  while(i <= size)
      ...
      if(strncmp(buf, "Dangbo", 6))
	{
	  ...
	  write(output,"\t", 1);
	  write(output,buf+2, strlen(buf+2));
	  sprintf(id,"\t%d", h[c].id);
	  write(output, id, strlen(id));
	  write(output, "\n", 1);
	  close(output);
	}
      ...
      if(i == size)
	{
      ...
      if(h[c+1].name == 0)
	{
	  date = increment_date(time, time->freq);
	  //	  printf("%s", date);
